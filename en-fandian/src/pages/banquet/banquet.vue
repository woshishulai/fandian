      ...
      <div class="reserve" v-show="yanhui">
        <div class="resfrom">
          <div class="resfrom_title">
            Fill your details here and we will get back to you
  ...
  methods: {
    ...
    geilistyu() {
      ...
      that.$axios
        // &module_id=3
        .post(
          `${this.Baseurl}feast?web_token=${that.token}&hotel_id=${that.hotel_id}`
        )
        .then(function (res) {
    ...
    getdatalist() {
      ...
      that.$axios
        ...
        .then(function (res) {
          ...
          //infolist
          //defaul_module_list
          that.infolist[1].news[0].pc_content = that.showHtml(
            that.infolist[1].news[0].pc_content
          );
          var array = that.infolist[0].news[0].images;
          var result = [];
          for (let i = 0; i < array.length; i++) {
            if (array[i].type == 1) result.push(array[i]);
          }
    ...
    tijiao() {
      ...
      if (this.datetime == "") {
        this.tishi("Please select date");
        return;
      }
      var re2 = /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
      if (!re2.test(this.email)) {
        this.tishi("Please enter the correct email address");
        return;
      }
      var re3 = /^[0-9]*[1-9][0-9]*$/;
      if (!re3.test(this.renshu)) {
        ///^((1[358][0-9])|(14[57])|(17[0678])|(19[7]))\d{8}$/
        this.tishi("Please enter the correct number of people");
        return;
      }
      ...
      that.$axios
        // &module_id=3
        .post(
          `${this.Baseurl}restaurant_sjjy?web_token=${that.token}&hotel_id=${that.hotel_id}
