          ...
          <div class="detailtwo">
            ...
            <div class="detailtwo_area" v-if="paricelist == ''">
              <p style="font-size: 20px; padding: 30px">
                No room type is available within the query period
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getidprice() {
      ...
      that.$axios
        .post(
          `${this.Baseurl}home_list?web_token=${that.token}&hotel_id=${that.hotel_id}
    ...
    yuding(item, index) {
      ...
      if (item.canSaleNum < 1) {
        that.tishi(
          "This property is insufficient, please change the room type or date"
        );
        console.log(item);
        return;
      }
